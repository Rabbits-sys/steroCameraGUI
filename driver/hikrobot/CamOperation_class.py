...
from driver.hikrobot.CameraParams_header import *
from driver.hikrobot.MvCameraControl_class import *
...
class CameraOperation:
    ...
    def Open_device(self):
        if not self.b_open_device:
            ...
            if ret != 0:
                return ret
            logger.info("open device successfully!")
            ...
            if ret != 0:
                logger.warning("get acquisition frame rate enable fail! ret[0x%x]", ret)
    ...
    def Start_grabbing(self, winHandle):
        if not self.b_start_grabbing and self.b_open_device:
            ...
            logger.info("start grabbing successfully!")
            try:
    ...
    def Stop_grabbing(self):
        if self.b_start_grabbing and self.b_open_device:
            ...
            if ret != 0:
                return ret
            logger.info("stop grabbing successfully!")
    ...
    def Close_device(self):
        ...
        self.b_start_grabbing = False
        self.b_exit = True
        logger.info("close device successfully!")
    ...
    def Set_parameter(self, frameRate, exposureTime, gain):
        if '' == frameRate or '' == exposureTime or '' == gain:
            logger.info('show info please type in the text box !')
        ...
        if self.b_open_device:
            ...
            if ret != 0:
                logger.error('show error set exposure time fail! ret = %s', To_hex_str(ret))
                return ret

            ret = self.obj_cam.MV_CC_SetFloatValue("Gain", float(gain))
            if ret != 0:
                logger.error('show error set gain fail! ret = %s', To_hex_str(ret))
                return ret

            ret = self.obj_cam.MV_CC_SetFloatValue("AcquisitionFrameRate", float(frameRate))
            if ret != 0:
                logger.error('show error set acquistion frame rate fail! ret = %s', To_hex_str(ret))
                return ret

            logger.info('show info set parameter success!')
    ...
    def Work_thread(self, winHandle):
        ...
        while True:
            ...
            if 0 == ret:
                ...
                logger.info("get one frame: Width[%d], Height[%d], nFrameNum[%d]", self.st_frame_info.nWidth, self.st_frame_info.nHeight, self.st_frame_info.nFrameNum)
                # 释放缓存
                self.obj_cam.MV_CC_FreeImageBuffer(stOutFrame)
            else:
                logger.warning("no data, ret = %s", To_hex_str(ret))
                continue
